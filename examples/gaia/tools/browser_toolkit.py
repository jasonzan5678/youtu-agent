...
from typing import Any, Callable, ClassVar, Dict, List, Optional, cast, Literal
...
class BrowserToolkit(BaseToolkit):
    ...
    def __new__(
        ...
        **kwargs: Any,
        ...
        if connect_over_cdp:
            raise ValueError("CDP connection is only supported in TypeScript mode")
        ...
        if viewport_limit:
            ...
            warnings.warn(
                "viewport_limit is not supported in Python mode and will be ignored",
                UserWarning,
            )
...
class HybridBrowserToolkit(BaseToolkit):
        ...
        try:
            ...
            try:
                ...
                if not loop.is_closed() and not loop.is_running():
                    ...
                    try:
                        loop.run_until_complete(asyncio.wait_for(self.browser_close(), timeout=2.0))
    ...
    def _load_unified_analyzer(self) -> str:
        ...
        script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "unified_analyzer.js")
    ...
    def _truncate_if_needed(self, content: Any) -> str:
        ...
        if self.max_log_length is not None and len(content_str) > self.max_log_length:
                ...
                if not error:
                    if isinstance(outputs, dict):
                        for key, value in outputs.items():
                            logger.info(f"  - {key}: {self._truncate_if_needed(value)}")
                    else:
                        logger.info(f"  Outputs: {self._truncate_if_needed(outputs)}")
    ...
    async def _get_session(self) -> "HybridBrowserSession":
        ...
        singleton = await HybridBrowserSession._get_or_create_instance(self._session)
    ...
    async def _wait_for_page_stability(self):
        ...
        try:
            ...
            await page.wait_for_load_state("domcontentloaded", timeout=self._page_stability_timeout)
            ...
            logger.debug(f"Page stability wait failed: {e} - continuing anyway")
    ...
    async def _get_unified_analysis(
        ...
        for attempt in range(max_retries):
            try:
                ...
                use_viewport_limit = viewport_limit if viewport_limit is not None else self._viewport_limit
                result = await page.evaluate(self._unified_script, use_viewport_limit)
                ...
                if attempt > 0:
                    logger.debug(f"Unified analysis succeeded on attempt {attempt + 1}")
                return result
                ...
                if is_navigation_error and attempt < max_retries - 1:
                    logger.debug(
                        f"Navigation error in unified analysis (attempt {attempt + 1}/{max_retries}): {e}. Retrying..."
                    )
                ...
                if attempt == max_retries - 1:
                    logger.warning(f"Error in unified analysis after {max_retries} attempts: {e}")
                else:
                    logger.warning(f"Non-retryable error in unified analysis: {e}")
    ...
    async def _get_tab_info_for_output(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"Successfully retrieved {len(tab_info)} tabs, current: {current_tab_index}")
            ...
            logger.warning(f"Failed to get tab info from session: {type(e).__name__}: {e}")
            ...
            try:
                ...
                session_state = {
                    ...
                    "pages_count": len(fallback_session._pages) if hasattr(fallback_session, "_pages") else "unknown",
                    "has_page": hasattr(fallback_session, "_page") and fallback_session._page is not None,
                    "session_id": getattr(fallback_session, "_session_id", "unknown"),
                }
                logger.debug(f"Browser session state: {session_state}")

                actual_tab_count = 0
                if hasattr(fallback_session, "_pages") and fallback_session._pages:
                    actual_tab_count = len(fallback_session._pages)
                    # Also try to filter out closed pages
                    try:
                        open_pages = [p for p in fallback_session._pages.values() if not p.is_closed()]
                        actual_tab_count = len(open_pages)
                        logger.debug(f"Found {actual_tab_count} open tabs out of {len(fallback_session._pages)} total")
                    except Exception:
                ...
                if actual_tab_count == 0:
                    ...
                    if hasattr(fallback_session, "_page") and fallback_session._page is not None:
                        actual_tab_count = 1
                        logger.debug("No pages in list but main page exists, assuming 1 tab")
                    else:
                        actual_tab_count = 1
                ...
                logger.warning(f"Fallback tab count also failed: {type(fallback_error).__name__}: {fallback_error}")
    ...
    async def _exec_with_snapshot(
        ...
        try:
            ...
            logger.info(f"Pre-action snapshot captured in {before_snapshot_time:.2f}s")
            ...
            if action_type in ["click", "type", "select", "enter"]:
                logger.info(f"Waiting for page stability after {action_type}...")
                stability_start = time.time()
                await self._wait_for_page_stability()
                stability_time = time.time() - stability_start
                logger.info(f"Page stability wait completed in {stability_time:.2f}s")
                ...
                if self.enable_page_loading_logging and self.log_to_console:
                    logger.info(f"[PAGE LOADING] Page stability for {action_type}: {round(stability_time * 1000, 2)}ms")
            ...
            logger.info(f"Post-action snapshot captured in {after_snapshot_time:.2f}s")
            ...
            if before_snapshot == after_snapshot:
                ...
                if "<empty>" in after_snapshot:
                    logger.warning(f"Action {action_type} resulted in empty snapshot - page may still be loading")
                elif len(after_snapshot.strip()) < 50:
                    logger.warning(f"Action {action_type} resulted in very short snapshot: {len(after_snapshot)} chars")
                else:
                    logger.debug(f"Action {action_type} resulted in updated snapshot: {len(after_snapshot)} chars")
            ...
            outputs = {
                ...
                "execution_stats": {
                    ...
                    "stability_time_ms": round(stability_time * 1000, 2) if stability_time > 0 else None,
            ...
            # log
            if snapshot == "snapshot not changed" and action_type == "click" and element_details:
                logger.debug("Snapshot unchanged after click. Adding element details to log.")
                outputs["clicked_element_tag"] = element_details.get("tagName", "N/A")
                outputs["clicked_element_content"] = element_details.get("name", "")
                outputs["clicked_element_type"] = element_details.get("role", "generic")
        ...
        try:
            ...
            if self.enable_action_logging or self.enable_timing_logging:
                ...
                await self._log_action(
                    ...
                    inputs=inputs,
                    outputs={"result": "Browser opened and navigated to default page."},
                    execution_time=execution_time,
                )
    ...
    async def browser_visit_page(self, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(tabs) == 1 and tabs[0].get("url") == "about:blank":
                logger.info("Found single blank tab, navigating in current tab instead of creating a new one.")
                should_create_new_tab = False
        except Exception as e:
            logger.warning(
                f"Could not get tab info to check for blank tab, proceeding with default behavior (new tab). Error: {e}"
            )
    ...
    async def browser_back(self) -> Dict[str, Any]:
        ...
        try:
            ...
            nav_start = time.time()
            await page.go_back(wait_until="domcontentloaded", timeout=self._navigation_timeout)
            ...
            logger.info(f"Back navigation snapshot captured in {snapshot_time:.2f}s")
    ...
    async def browser_forward(self) -> Dict[str, Any]:
        ...
        try:
            ...
            nav_start = time.time()
            await page.go_forward(wait_until="domcontentloaded", timeout=self._navigation_timeout)
            nav_time = time.time() - nav_start
            ...
            snapshot = await self._session.get_snapshot(force_refresh=True, diff_only=False)
            snapshot_time = time.time() - snapshot_start
            logger.info(f"Forward navigation snapshot captured in {snapshot_time:.2f}s")
    ...
    async def browser_get_page_snapshot(self) -> str:
        ...
        logger.info(f"Page snapshot analysis completed in {analysis_time:.2f}s")
    ...
    async def browser_get_som_screenshot(
        ...
        logger.info(f"Starting screenshot capturewith timeout: {self._screenshot_timeout}ms")
        ...
        file_path = os.path.join(self._cache_dir, f"{url_name}_{timestamp}_som.png")
        marked_image.save(file_path, "PNG")

        text_result = f"Visual webpage screenshot captured with {len(rects)} interactive elements."
    ...
    async def browser_click(self, *, ref: str) -> Dict[str, Any]:
        ...
        result = await self._exec_with_snapshot(action, element_details=element_details)
    ...
    @action_logger
    async def browser_mouse_control(self, *, control: str, x: float, y: float) -> Dict[str, Any]:
        ...
        if control not in ("click", "right_click", "dblclick"):
            tab_info = await self._get_tab_info_for_output()
            return {
                "result": "Error: supported control actions are 'click' or 'dblclick'",
                "snapshot": "",
                **tab_info,
            }
    ...
    @action_logger
    async def browser_mouse_drag(self, *, from_ref: str, to_ref: str) -> Dict[str, Any]:
        ...
        if from_ref not in elements:
            logger.error(f"Error: Source element reference '{from_ref}' not found.")
            snapshot = self._format_snapshot_from_analysis(analysis)
            tab_info = await self._get_tab_info_for_output()
            return {
                "result": (f"Error: Source element reference '{from_ref}' not found."),
                "snapshot": snapshot,
                **tab_info,
            }

        if to_ref not in elements:
            logger.error(f"Error: Target element reference '{to_ref}' not found.")
            snapshot = self._format_snapshot_from_analysis(analysis)
            tab_info = await self._get_tab_info_for_output()
            return {
                "result": (f"Error: Target element reference '{to_ref}' not found."),
                "snapshot": snapshot,
                **tab_info,
            }
    ...
    @action_logger
    async def browser_wait_user(self, timeout_sec: Optional[float] = None) -> Dict[str, Any]:
        ...
        prompt = "🕑 Agent waiting for human input. Complete action in browser, then press Enter..."
        logger.info(f"\n{prompt}\n")
        ...
        try:
            if timeout_sec is not None:
                logger.info(f"Waiting for user input with timeout: {timeout_sec}s")
                start_time = time.time()
            ...
            logger.info(f"User input timeout reached after {wait_time}s, auto-resuming")
    ...
    async def browser_console_exec(self, code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            snapshot = await self._session.get_snapshot(force_refresh=True, diff_only=False)
            snapshot_time = time.time() - snapshot_start
            logger.info(f"Code execution snapshot captured in {snapshot_time:.2f}s")
    ...
    def get_log_summary(self) -> Dict[str, Any]:
        ...
        total_execution_time = sum(entry.get("execution_time_ms", 0) for entry in self.log_buffer)
        total_page_load_time = sum(
            entry.get("page_load_time_ms", 0) for entry in self.log_buffer if "page_load_time_ms" in entry
        )
        ...
        return {
            ...
            "success_rate": round((total_actions - error_count) / total_actions * 100, 2) if total_actions > 0 else 0,
        }
    ...
    def clone_for_new_session(self, new_session_id: Optional[str] = None) -> "HybridBrowserToolkit":
    ...
    async def browser_switch_tab(self, *, tab_id: str) -> Dict[str, Any]:
        ...
        if success:
            ...
            tab_info = await self._get_tab_info_for_output()
            result = {
                "result": f"Failed to switch to tab {tab_id}. Tab may not exist.",
                "snapshot": "",
                **tab_info,
            }
    ...
    def get_tools(self) -> List[FunctionTool]:
        ...
        for tool_name in self.enabled_tools:
            if tool_name == "browser_solve_task" and self._web_agent_model is None:
                logger.warning(
                    f"Tool '{tool_name}' is enabled but web_agent_model is not provided. Skipping this tool."
                )
                continue

            if tool_name in tool_map:
                tool = FunctionTool(cast(Callable[..., Any], tool_map[tool_name]))
